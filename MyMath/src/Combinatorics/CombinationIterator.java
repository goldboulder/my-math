...
public class CombinationIterator<T> implements Iterator<LinkedList<T>>{
    ...
    //interface for different kinds of iterators?
    ...
    public CombinationIterator(T[] iterable, int numElements){
        //convert to linkedList
        this.list = new LinkedList<>();
        list.addAll(Arrays.asList(iterable));
        
        initiate(numElements);
    }
    
    public CombinationIterator(Iterable<T> iterable, int numElements){
        //convert to linkedList
        this.list = new LinkedList<>();
    ...
    private void initiate(int numElements){
        this.numElements = numElements;
        guarenteedIndex = numElements - 1;
        numLeft = MyMath.nCr(list.size(),numElements);
